...
fn deconstruct_f32(n: f32) -> (u32,u32,u32) {
    let _n: u32 = unsafe {std::mem::transmute(n) };
    //Strip 31 bits away by shifting them into the void, leaving the sign bit
    let sign = (_n>>31)&1;
    //Strip 23 bits away by shifting them in the void, leaving the exponent bit
    let exponent = (_n>>23) & 0xff;
    //Retain the least signifigant 23 bits via an and mask.
    let fraction = 0b00000000_01111111_11111111_11111111 & _n;
